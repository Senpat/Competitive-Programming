...
public class P1696{
   ...
   public static void main(String[] args)throws IOException{
      ...
      MaxFlow flow = new MaxFlow(b+g+2,source,sink);
      
      MaxFlow.Edge[] edges = new MaxFlow.Edge[p];
      for(int k = 0; k < p; k++){
         st = new StringTokenizer(f.readLine());
         int bi = Integer.parseInt(st.nextToken());
         int gi = Integer.parseInt(st.nextToken());
         
         edges[k] = flow.addEdge(bi,gi+b,1L);
      }
      ...
      for(int k = 0; k < p; k++){
         if(edges[k].flow == 1L){
            sj.add("" + edges[k].from + " " + (edges[k].to-b));
         }
      }
