#include <bits/stdc++.h>
// based on P2102.cpp, but cleaned up
// finds every match, runs in time for worst case (string is aaaa... and patterns are a, aa, aaa, etc.)
// because total length of patterns is < 5e5 and in that worst case it will run in sqrt(N)^2 = N.
using namespace std;
...
struct Trie{
   Trie* children[26];
   int i;                  //index of pattern, -1 if it is not a pattern
   bool reached;
   Trie* parent;
   int pchi;
