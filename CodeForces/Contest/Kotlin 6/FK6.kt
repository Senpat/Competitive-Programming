...
fun main(){
	...
	//index of closest greater or equal number to the left
	val stk = Stack<Int>()
	val greaterleft = IntArray(n){0}
	for(k in n-1 downTo 0){
		while(!stk.isEmpty() && array[stk.peek()!!] < array[k]){
			val i = stk.pop()!!
			greaterleft[i] = k+1
		}
		stk.push(k)
	...
	//println(greaterleft.joinToString(" "))

	val lst = SegmentTreeFast(n)

	var r = 0

	val answer = LongArray(q){0}
	for(k in 0 until q){
		while(r <= queries[k].second){
			lst.modify(greaterleft[r],r,array[r])
			/*
			for(k in 0 until n){
				print("${lst.query(k,k)} ")
			}
		...
		answer[queries[k].third] = lst.query(queries[k].first,queries[k].second)
	}
