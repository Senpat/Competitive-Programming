...
public class C666{
   ...
   public static void main(String[] args)throws IOException{
      ...
      r3 = Long.parseLong(st.nextToken());
      ...
      for(int k = 1; k < n; k++){
         ...
         dp[k][0] = Math.min(Math.min(dp[k-1][0],dp[k-1][1])+ngb,dp[k-1][1] + Math.min(gb,ngb));
         //if(k == n-1) dp[k][0] = Math.min(dp[k][0],dp[k-1][1] + Math.min(gb,ngb)-d);
         dp[k][1] = dp[k-1][0] + gb + 2L*d;
         if(k == n-1){
            dp[k][2] = dp[k-1][2] + Math.min(ngb,gb+2L*d);
         } else {
      ...
      long answer = Math.min(dp[n-1][0],Math.min(dp[n-1][1],dp[n-1][2])) + (long)(n-1)*d;
      out.println(answer);
