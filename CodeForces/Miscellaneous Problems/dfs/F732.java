...
public class F732{
   ...
   public static void dfs2(int v){
      ...
      for(int ei : adj.get(v)){
         int eto = edges[ei][0] + edges[ei][1] - v;
         if(seen[eto] || numbackedges[ei] == 0) continue;       //skip edge if edge is bridge or node is seen
         seen[eto] = true;
         dfs2(eto);
      }
   }
   
   public static void calcbridges(int v){
      if(parentedge[v] != -1){
      ...
      for(int ei : adj.get(v)){
         int eto = edges[ei][0] + edges[ei][1] - v;
         if(ei == parentedge[v] || isbackedge[ei]) continue;
