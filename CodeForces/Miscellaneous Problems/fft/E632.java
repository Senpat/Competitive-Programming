...
public class E632{
   ...
   public static void main(String[] args)throws IOException{
      ...
      long[] answer = polyexp(array,m);
      
      for(int k = 1; k < answer.length; k++){
         if(answer[k] > 0L) out.print(k + " ");
      }
   ...
   public static long[] polyexp(long[] array, int power){
      ...
      while(p > 0){
         ...
         //for(int k = 0; k < ans.length; k++) ans[k] = Math.min(ans[k],1);                 //MAKES EVERYTHING 0 OR 1
      }
   ...
   public static long[] mul(long[] a,long[] b) {
      ...
      for(int k = 0; k < a.length; k++) a[k] = Math.min(a[k],1);
      return a;
   }
