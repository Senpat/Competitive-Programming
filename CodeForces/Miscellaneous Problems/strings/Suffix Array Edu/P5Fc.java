...
import java.util.*;
//hint (cnblogs)
//based on P5Fb, different method for finding front extra part
public class P5Fc{
   ...
   public static void main(String[] args)throws IOException{
      ...
      for(int gap = 1; gap <= n; gap++){
         for(int k = 0; k+gap <= n; k += gap){
            ...
            int baselcp = getmin(a,b-1);
            
            answer = Math.max(answer,baselcp/gap+1);
            
            int l = 1;
            int r = Math.min(gap,k);
            int ans = baselcp;
            
            while(l <= r){
               int mid = l + (r-l)/2;
               
               a = Math.min(indexof[k-mid],indexof[k+gap-mid]);
               b = Math.max(indexof[k-mid],indexof[k+gap-mid]);
            ...
            answer = Math.max(answer,ans/gap+1);
