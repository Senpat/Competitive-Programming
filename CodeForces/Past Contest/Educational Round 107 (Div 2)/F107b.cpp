#include <bits/stdc++.h>
//further optimize, states with same suffixes can be combined into 1 state
//debug: find out ways to combine states from ~260 states worse case to <200 worse case
using namespace std;
...
int main(){
   ...
   map<pair<string,string>,int> suffs;
   ...
   for(int s2 = 0; s2 < n; s2++){
      ...
      for(int i2 = 0; i2 < s[s2].length(); i2++){
         ...
         if(!fail){
            ...
            pair<string,string> suff = {s[s1].substr(i1+1),s[s2].substr(i2+1)};
            if(suffs.find(suff) != suffs.end()){
               indexof[cur] = suffs[suff];
            } else {
               indexof[cur] = states.size();
               suffs[suff] = states.size();
