...
//returns points in smallest convex hull in counterclockwise order
...
vector<Point> convexhull(vector<Point>& points){
   ...
   for(int k = 0; k < n; k++){
      while(stk.size() >= 2){
         ...
         Point p1 = stk.top();
         if(crossproductz(pdiff(p2,p1),pdiff(points[k],p1)) < 0LL){
            //correct
            stk.push(p2);
            break;
         }
...
int main(){
   ...
   while(true){
      ...
      vector<Point> answer = convexhull(points);
