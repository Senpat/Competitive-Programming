...
public class EMBIT{
   ...
   public static void main(String[] args)throws IOException{
      ...
      for(int k = 1; k < 1000; k++){
         spread[k] = (int)Math.floor(1.0/d * (double)k);
      }
      ...
      for(int k = 1; k <= m; k++){
         dp[0][k] = Math.abs(k-points[0].x);
         update(1,1,m,k,dp[0][k],0);
      }
      ...
      for(int k = 1; k < n; k++){
         for(int j = 1; j <= m; j++){
            ...
            //out.flush();
            int min = min(1,1,m,Math.max(1,j-spread[points[k].y-points[k-1].y]),Math.min(m,j+spread[points[k].y-points[k-1].y]),k-1);
            
            int newvalue = min + Math.abs(j-points[k].x);
            dp[k][j] = newvalue;
            update(1,1,m,j,dp[k][j],k);
         }
      }
