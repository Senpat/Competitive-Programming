...
import java.util.*;
//still too slow
public class magicalmysterysort{
   ...
   public static void dothing(int x, int y, int i){
      ...
      if(i == 64){
         ...
         if(given.contains(i+1)){
            ...
            ArrayList<State> neighbors = new ArrayList<State>(8);
            for(int k = 0; k < 8; k++){
               if(in(x+dx[k],y+dy[k]) && board[x+dx[k]][y+dy[k]] == -1 && rowsum[x+dx[k]] + 65-(i+1) <= 260 && colsum[y+dy[k]] + 65-(i+1) <= 260){
                  neighbors.add(new State(x+dx[k],y+dy[k],countnei(x,y,i+1)));
               }
            }
            
            Collections.sort(neighbors);
            for(State nei : neighbors){
               dothing(nei.x,nei.y,i+1);
            }
         }
   ...
   public static int countnei(int x, int y, int i){
      if(given.contains(i+1)) return 1;
      
      int ret = 0;
      for(int k = 0; k < 8; k++){
         if(in(x+dx[k],y+dy[k]) && board[x+dx[k]][y+dy[k]] == -1) ret++;
      }
      return ret;
   }
